...
export default class CardsList {
  ...
  #createCard(
    { urlPhoto, category, title, text, date, url, alt, id, isFavorite },
    order
  ) {
    return `
        <li class="card" style="order: ${order}" id="${id}">
            ...
            <button class="card-button add-favorite-btn ${this.#visibleAddToFavorite(
              isFavorite
            )}" type="button">Add to favorite
            ...
            <button class="card-button remove-favorite-btn ${this.#visibleRemoveFromFavorite(
              isFavorite
            )}" type="button">Remove from favorite
                <svg class="card-button-icon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
